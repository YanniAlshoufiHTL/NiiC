...
function showAetInputPrompt() {
    ...
    const dateEl = aetInputPrompt.querySelector(".niic-aet-input-date");
...
function showAetEditPrompt(id) {
    ...
    const aet = aets[idx];
    const titleEl = document.querySelector(".niic-aet-input-prompt-title");
    titleEl.value = aet.title;
    const descriptionEl = document.querySelector(".niic-aet-input-prompt-description-textarea");
    descriptionEl.textContent = aet.description;
    const timeBtnEl = document.querySelector(".niic-aet-btn-time");
    const timeToStr = (time) => `${(Math.floor(time).toString()).padStart(2, "0")}:${(((time - Math.floor(time)) * 60)).toString().padStart(2, "0")}`;
    timeBtnEl.innerText = `${timeToStr(aet.startTime)} - ${timeToStr(aet.endTime)}`;
    const dateEl = document.querySelector(".niic-aet-input-date");
    dateEl.value = dateToInputStr(aet.date);
    const typeEl = document.querySelector(".niic-aet-type-select");
    typeEl.value = aet.type;
    const colorEl = document.querySelector(".niic-color-btn");
    let color = aet.color;
...
function submitAetInputPrompt(ev) {
    ev.preventDefault();
    const promptEl = document.querySelector(".niic-aet-input-prompt");
    const id = promptEl && promptEl.dataset.id && /^\d+$/.test(promptEl.dataset.id)
        ? +promptEl.dataset.id
        : false;
    const title = document.querySelector(".niic-aet-input-prompt-title")?.value;
    const description = document.querySelector(".niic-aet-input-prompt-description-textarea")?.textContent;
    const date = document.querySelector(".niic-aet-input-date")?.valueAsDate;
    const timeStr = document.querySelector(".niic-aet-btn-time")?.textContent?.trim();
    const type = document.querySelector(".niic-aet-type-select")?.value;
    const color = document.querySelector(".niic-color-btn")?.value;
